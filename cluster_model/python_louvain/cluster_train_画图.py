    ...
    def train_model(self, config_path):
        ...
        for r in resolution_range:
            ...
            with open("/home/yuanshuai20/paper/互作对选择resolution.txt", "w") as f1:
                ...
                f1.write(str(resolution_col1) + '\n')
                f1.write(str(resolution_col2) + '\n')
            ...
            resolution_col1.append(df['col1'].nunique())
            resolution_col2.append(df['col2'].nunique())

        fig, axs = plt.subplots(2, 2, figsize=(10, 10))
        axs[0][0].plot(resolution_range, resolution_communities)
        axs[0][0].set_xlabel('resolution')
        axs[0][0].set_ylabel('簇数')

        axs[1][0].plot(resolution_range, resolution_col1)
        axs[1][0].set_xlabel('resolution')
        axs[1][0].set_ylabel('稻瘟菌节点数')

        axs[1][1].plot(resolution_range, resolution_col2)
        axs[1][1].set_xlabel('resolution')
        axs[1][1].set_ylabel('水稻节点数')

        axs[0][1].plot(resolution_range, resolution_mq)
        axs[0][1].set_xlabel('resolution')
        axs[0][1].set_ylabel('模块度')
        plt.show()
