...
import os
import sys
sys.path.append("/home/yuanshuai20/paper/gae_model/")
from configparser import ConfigParser
from model.mymodel import NHGATModelGAN
from model import define_my_optimizer
from util.load_data import load_data, sparse_to_tuple, mask_test_edges, preprocess_graph
from model.loss import arga_loss_function, varga_loss_function
from util.metrics import get_roc_score
import model.dimension_reduction as dr
...
class Train():
    ...
    def train_model(self, config_path):
        if os.path.exists(config_path) and (os.path.split(config_path)[1].split('.')[0] == 'config') and (
            ...
            hidden_dim4 = config.getint(section, 'hidden_dim4')
            hidden_dim5 = config.getint(section, 'hidden_dim5')
            m_seed = config.getint(section, 'm_seed')
            ...
            ##创建一个大小为 num_nodes 的单位矩阵
            features = sparse_to_tuple(sp.identity(num_nodes))
            num_features = features[2][1]
            # 将一个矩阵的对角线元素减去，然后去除矩阵中的零元素
            ...
            '''
            首先，使用 preprocess_graph 函数对原始邻接矩阵进行预处理，得到一个归一化的邻接矩阵 adj_norm。
            然后，将训练集的邻接矩阵 adj_train 加上一个单位矩阵，得到一个新的矩阵 adj_label。
            最后，将 adj_label 转换为 PyTorch 的 FloatTensor 类型，并将其转移到设备上（即 GPU 或 CPU）。
            '''
            ...
            print('create gae_model ...')
            model = NHGATModelGAN(num_features, hidden_dim1=hidden_dim1, hidden_dim2=hidden_dim2,
                                  hidden_dim3=hidden_dim3, hidden_dim4=hidden_dim4,hidden_dim5=hidden_dim5,
                                  num_heads=num_heads, dropout=dropout, alpha=alpha, vae_bool=vae_bool)

            model = model.to(DEVICE)
            # define optimizer
            if optimizer_name == 'adam':
                optimizer = define_my_optimizer.define_optimizer_adam(model, lr=lr, weight_decay=weight_decay)

            elif optimizer_name == 'adamw':
                optimizer = define_my_optimizer.define_optimizer_adamw(model, lr=lr, weight_decay=weight_decay)

            elif optimizer_name == 'sgd':
                optimizer = define_my_optimizer.define_optimizer_sgd(model, lr=lr, momentum=momentum,
                                                                  weight_decay=weight_decay)

            elif optimizer_name == 'adagrad':
                optimizer = define_my_optimizer.define_optimizer_adagrad(model, lr=lr, lr_decay=lr_decay,
                                                                      weight_decay=weight_decay)

            elif optimizer_name == 'rmsprop':
                optimizer = define_my_optimizer.define_optimizer_rmsprop(model, lr=lr, weight_decay=weight_decay,
                                                                      momentum=momentum)

            elif optimizer_name == 'adadelta':
                optimizer = define_my_optimizer.define_optimizer_adadelta(model, lr=lr, weight_decay=weight_decay)
            ...
            for epoch in range(epochs):
                ...
                optimizer.zero_grad()
                # 解码后的邻接矩阵，判别器
                recovered, dis_real, dis_fake, mu, logvar,z_fake = model(features, adj_norm)
                if vae_bool:
            ...
            index = pd.Index(node_name_list)
            df = pd.DataFrame(hidden_emb, index=index)
            # save the dataframe as a csv file
            df.to_csv(nodes_path)
            ...
            print('开始降维')
            dismension = dr.run_dimension(hidden_emb,2)
